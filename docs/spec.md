...
**Retries:** transient errors (timeouts, 5xx, GitHub rate limit) get 3 attempts with exponential backoff (1s/2s/4s) at the tool-call level. Validation/not-found/access-denied errors fail fast, no retry. SQS redrive policy sends a job to a DLQ after 3 receive attempts; DLQ arrival triggers an alert and the job is marked `failed`.
...
- **Stack:** Prometheus + Grafana + Loki/Promtail, self-hosted in the `monitoring` namespace (one Grafana pane for metrics and logs), plus native CloudWatch metrics/alarms for AWS-managed resources (EC2, DynamoDB throttles, SQS queue age/DLQ count, S3 errors).
...
- **E2E:** fixture repo + a stub LLM client returning canned JSON (not live Claude calls) — deterministic, fast, no API cost or flakiness in CI. Asserts the final coverage matrix end-to-end through the UI.
