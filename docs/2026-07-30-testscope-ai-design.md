...
- **IAM:** the worker EC2 instance (where all application pods are scheduled — the control-plane's default `NoSchedule` taint is left in place) carries an attached IAM instance profile; pods get AWS access via instance-metadata credentials — no static AWS keys stored as k8s Secrets. Policy scoped to the specific per-env S3 buckets/DynamoDB tables/SQS queues below. The control-plane node runs no application workloads and does not need this profile. `dev`/`prod`'s `iam` module attaches its policy to that role **by a hardcoded name literal** (`testscope-k8s-worker-role`, matching `environments/shared`'s fixed naming), not via a `terraform_remote_state` lookup into `shared`'s state — a deliberate choice to keep `dev`/`prod` decoupled from `shared`'s state file (no environment needs read access to another environment's state on disk), at the cost of the role name being a convention the two sides have to agree on rather than something Terraform enforces structurally. Revisit if `shared`'s role naming ever needs to change per-environment.
