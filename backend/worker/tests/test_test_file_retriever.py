...
async def test_populates_candidate_files():
    ...
    with patch("worker_app.nodes.test_file_retriever.call_test_mcp_tool", new=AsyncMock(return_value=fake_response)):
...
async def test_no_matches_is_not_a_failure():
    ...
    with patch("worker_app.nodes.test_file_retriever.call_test_mcp_tool", new=AsyncMock(return_value={"files": []})):
        result = await test_file_retriever(state)
...
async def test_search_failure_is_non_fatal():
    state = {"repository": "acme/widgets", "default_branch": "main", "analysis_id": "a1",
             "search_keywords": ["login"], "tool_call_trace": [], "warnings": []}
    with patch("worker_app.nodes.test_file_retriever.call_test_mcp_tool", new=AsyncMock(side_effect=Exception("mcp-test-analysis unreachable"))):
        result = await test_file_retriever(state)
    assert result["candidate_files"] == []
    assert result.get("status") != "failed"
